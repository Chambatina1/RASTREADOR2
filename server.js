...
// ================= BASE MANUAL (RAW_TRACKING_SOURCE) =================
...
CHAMBATINA MIAMI	GEO MIA		CPK-0265026	EN AGENCIA	No	ENVIOS FACTURADOS	ENVIOS FACTURADOS/()/(ENVIOS FACTURADOS)	ENVIO	MISCELANEAS		2026-04-09	TAMARA CABEZA MUNOZ		75012200277	CALLE J # 453 A ALTOS Rpto. VEDADO e/ 21 y 23, PLAZA, LA HABANA	52460783	YORDANY OVALLE			2.99	0	1	51.8	4.125	0.5	0	0		
...
CHAMBATINA MIAMI	GEO MIA		CPK-0233751	ENTREGADO	Sí	045(CPK-287)	DENEB/(SEGU 5662673)/(CWPS26138502)	ENVIO	LAVADORA/ SECADORA AUTOMATICA MODELO LG	9902	2026-01-12	ELSA BARRIOS PEREZ		86012204812	AVE 25 # 3017 Rpto. LA SIERRA e/ 30 y 34, PLAYA, LA HABANA	53358593	ERISBEL FORNARIS			0	0	1	335	4.63	0	0	0		
CHAMBATINA MIAMI	GEO MIA		CPK-0255137	ENTREGADO	Sí	140(CPK-309)	REGULA/(BSIU 9722526)/(CWPS26167603)	ENVIO	MISCELANEAS	10916	2026-03-09	ELSA BARRIOS PEREZ		86012204812	AVE 25 # 3017 Rpto. LA SIERRA e/ 30 y 34, PLAYA, LA HABANA	53358593	ERISBEL FORNARIS			2.99	0	1	34.9	0.579	0.5	0	0
CHAMBATINA MIAMI	GEO MIA		CPK-0260443	EN AGENCIA	No	ENVIOS FACTURADOS	ENVIOS FACTURADOS/()/(ENVIOS FACTURADOS)	ENVIO	MISCELANEAS		2026-03-26	DIANARA CORREA SANCHEZ		94092744494	Ave Piti fajardo # Edificio 27 Apto 4 Rpto. Emilio Barcenas e/ 9 y 11, HOLGUIN, HOLGUIN	52065680	YISEL LOPEZ ALVAREZ			2.99	0	1	32	3.375	0.5	0	0		
CHAMBATINA MIAMI	GEO MIA		CPK-0260440	EN AGENCIA	No	ENVIOS FACTURADOS	ENVIOS FACTURADOS/()/(ENVIOS FACTURADOS)	ENVIO	MISCELANEAS		2026-03-26	YURISLEIDI TAPIA ALVAREZ		92110940512	CALLE 16 # 112 E Rpto. PIEDRECITA e/ 13 y 15, CESPEDES, CAMAGUEY	56244435	YISEL LOPEZ ALVAREZ			2.99	0	1	26	3.375	0.5	0	0		
...
app.get("/api/health", (req, res) => {
  try {
    return res.json({
      ...
      totalCPK: RAW_TRACKING_SOURCE.split("\n").filter(l => l.includes("CPK-")).length
    });
  } catch (error) {
...
// ================= RASTREO POR CPK (manual) =================
app.get("/api/rastreo/:cpk", (req, res) => {
  try {
    const cpkBuscado = normalizarCPK(req.params.cpk);
    ...
    for (const linea of lineas) {
      if (linea.includes(`CPK-${cpkBuscado}`)) {
        const partes = linea.split("\t");
        encontrado = {
          cpk: cpkBuscado,
          estado: partes[4] || "SIN ESTADO",
          fecha: partes[11] || "",
          descripcion: partes[9] || "Sin descripción",
          embarcador: partes[12] || "",
          consignatario: ""
        };
        break;
      }
    ...
    return res.json({
      ok: true,
      ...encontrado,
      saludo: construirSaludo(encontrado.embarcador, encontrado.consignatario, encontrado.estado)
    });
  } catch (error) {
  ...
  try {
    ...
    if (!carnet || carnet.length !== 11) {
      return res.status(400).json({
        ok: false,
        message: "Carnet inválido (debe tener 11 dígitos)",
        results: []
      });
    }
    ...
    for (const linea of lineas) {
      if (linea.includes(carnet)) {
        const partes = linea.split("\t");
        resultados.push({
          nombre: partes[12] || "",
          carnet: carnet,
          cpk: (partes[3] || "").replace("CPK-", ""),
          estado: partes[4] || "",
          fecha: partes[11] || "",
    ...
    if (resultados.length === 0) {
      return res.status(404).json({
        ok: false,
        message: "No se encontraron envíos para ese carnet",
        results: []
      });
    }
...
app.get("/api/buscar/:termino", async (req, res) => {
  try {
    ...
    // Buscar como CPK
    const lineas = RAW_TRACKING_SOURCE.split("\n");
    let encontradoCPK = null;

    for (const linea of lineas) {
      if (linea.includes(`CPK-${termino}`)) {
        const partes = linea.split("\t");
        encontradoCPK = {
          cpk: termino,
          estado: partes[4] || "SIN ESTADO",
    ...
    if (encontradoCPK) {
      return res.json({
        ...
        tipoBusqueda: "cpk",
        ...encontradoCPK,
        saludo: construirSaludo(encontradoCPK.nombre, "", encontradoCPK.estado)
      });
    }
    // Buscar como carnet
    const resultadosCarnet = [];
    for (const linea of lineas) {
      if (linea.includes(termino) && linea.includes("CPK-")) {
        const partes = linea.split("\t");
        resultadosCarnet.push({
          cpk: (partes[3] || "").replace("CPK-", ""),
          estado: partes[4] || "",
          fecha: partes[11] || "",
          descripcion: partes[9] || "",
...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    if (info.intent === "rastreo" && info.cpk) {
      ...
      let encontrado = null;
      for (const linea of lineas) {
        if (linea.includes(`CPK-${info.cpk}`)) {
          const partes = linea.split("\t");
          encontrado = {
            cpk: info.cpk,
            estado: partes[4] || "SIN ESTADO",
      ...
      return res.json({
        ok: true,
        respuesta:
          `${construirSaludo(encontrado.embarcador, encontrado.consignatario, encontrado.estado)}\n\n` +
          `Fecha: ${encontrado.fecha || "No disponible"}\n\n` +
    ...
    // Continuar cálculo si el usuario responde "y con eso" etc.
    if (
      !info.peso &&
    ...
    // Si no hay API key de OpenAI, responder con mensaje por defecto
    if (!process.env.OPENAI_API_KEY) {
      return res.json({
        ok: true,
        respuesta: "Lo siento, no puedo responder preguntas generales en este momento. Por favor, escribe un CPK, un carnet o una pregunta sobre precios y servicios."
      });
    }
